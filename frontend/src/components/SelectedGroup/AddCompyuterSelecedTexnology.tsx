...
type Props = {
  ...
  selectedIdComp: number | undefined;
  selectedTexnologyId: React.Dispatch<React.SetStateAction<number | null>>

};

export default function AddCompyuterSelecedTexnology({ label, selectData, selectedIdComp, selectedTexnologyId }: Props) {
  ...
  // Agar `selectedIdComp` o‘zgaradigan bo‘lsa, mos keladigan elementni tanlash
  useEffect(() => {
    if (selectedIdComp) {
      const selectedItem = selectData.find((item) => item.id === selectedIdComp);
      if (selectedItem) {
