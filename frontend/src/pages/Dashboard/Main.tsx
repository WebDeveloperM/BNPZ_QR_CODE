      ...
      {data ?
        <>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
            <CardDataStats title="Компьютеры" total={`${infoCompData?.all_compyuters_count}`} levelUp>
              <FaComputer className="fill-primary dark:fill-white" width="20" height="22" />
            </CardDataStats>

            <CardDataStats title="Принтеры" total={`${infoCompData?.all_compyuters_with_printer}`} levelUp>
              <AiOutlinePrinter className="fill-primary dark:fill-white" width="20" height="22" />
            </CardDataStats>

            <CardDataStats title="Сканеры" total={`${infoCompData?.all_compyuters_with_scaner}`} levelUp>
              <MdOutlineAdfScanner className="fill-primary dark:fill-white" width="20" height="22" />
            </CardDataStats>

            <CardDataStats title="Веб-камеры" total={`${infoCompData?.all_compyuters_with_webcam}`} levelDown>
